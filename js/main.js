...
function updateFormationPositions() {
  ...
  game.player.units.forEach(u => {
    ...
    if (u.isMiner) {
      u.formationX = W * 0.02 + midx * 18;
      u.formationY = py + 8;
      midx++;
    } else {
      u.formationX = W * 0.08 + Math.floor(sidx / 5) * 28 + (sidx % 5) * 20;
      u.formationY = py - Math.floor(sidx / 5) * 4;
      sidx++;
    }
  });
  let emidx = 0, esidx = 0;
  game.enemy.units.forEach(u => {
    if (u.state === 'dead' || u.state === 'dying') return;
    if (u.isMiner) {
      u.formationX = W * 0.96 - emidx * 18;
      u.formationY = py + 8;
      emidx++;
    } else {
      u.formationX = W * 0.88 - Math.floor(esidx / 5) * 28 - (esidx % 5) * 20;
